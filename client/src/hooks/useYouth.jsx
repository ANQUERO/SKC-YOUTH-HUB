...
const useYouth = () => {
  ...
  const updateYouth = async (youth_id, updateData) => {
    ...
    try {
        ...
        if (youth) {
          setYouth((prevYouth) => ({
            ...
            name: updateData.name
              ? { ...prevYouth.name, ...updateData.name }
              : prevYouth.name,
            location: updateData.location
              ? { ...prevYouth.location, ...updateData.location }
              : prevYouth.location,
            info: updateData.info
              ? { ...prevYouth.info, ...updateData.info }
              : prevYouth.info,
            demographics: updateData.demographics
              ? { ...prevYouth.demographics, ...updateData.demographics }
              : prevYouth.demographics,
            survey: updateData.survey
              ? { ...prevYouth.survey, ...updateData.survey }
              : prevYouth.survey,
            meetingSurvey: updateData.meetingSurvey
              ? { ...prevYouth.meetingSurvey, ...updateData.meetingSurvey }
              : prevYouth.meetingSurvey,
          }));
        }
      ...
      setError(
        error.response?.data?.message || "Failed to update youth profile",
      );
      return null;
    } finally {
      setLoading(false);
