  ...
  try {
    const result = await pool.query(`
            SELECT
                ...
                a.file_url
            FROM sk_official o
            LEFT JOIN sk_official_name n ON n.official_id = o.official_id
            LEFT JOIN sk_official_info i ON i.official_id = o.official_id
            LEFT JOIN sk_official_avatar a ON a.official_id = o.official_id
...
export const update = async (req, res) => {
  ...
  if (
    ...
    parseInt(official_id, 10) !== user.official_id
  ) {
    return res.status(403).json({
      status: "Error",
  ...
  try {
    const allowedFields = ["email", "official_position"];
  ...
  try {
    const result = await pool.query(
      `INSERT INTO landing_page_content
             (official_id, official_name, official_title, media_type, media_url)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`,
      [user.official_id, official_name, official_title, media_type, media_url],
    );
...
export const deleteContent = async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM landing_page_content WHERE content_id = $1", [
      parseInt(content_id),
    ]);
