...
export const createCommentReaction = async (req, res) => {
    const user = req.user;
    ...
    try {
        ...
        const check = await pool.query(
            `
            SELECT reaction_id FROM comment_reactions
            WHERE comment_id = $1 AND user_type = $2 AND user_id = $3
            `,
            [comment_id, user.userType, authorId]
        );
        ...
        if (check.rows.length > 0) {
            ...
            const insert = await pool.query(
                `
                INSERT INTO comment_reactions (comment_id, user_type, user_id, type)
                VALUES ($1, $2, $3, $4)
                RETURNING *
                `,
                [comment_id, user.userType, authorId, type]
            );
            reaction = insert.rows[0];
        }
...
// Get all reactions of a comment
