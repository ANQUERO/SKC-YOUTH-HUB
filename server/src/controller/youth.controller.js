...
export const show = async (req, res) => {
  ...
  const isSelf =
    user.userType === "youth" && user.youth_id === parseInt(youth_id);
...
export const update = async (req, res) => {
  ...
  if (!isSelf && !isAdmin) {
    return res.status(403).json({
      status: "Error",
      message:
        "Forbidden - You can only update your own profile or must be an admin",
    });
  }
  ...
  try {
    ...
    if (updateData.location && Object.keys(updateData.location).length > 0) {
      ...
      if (
        updateData.location.purok !== undefined ||
        updateData.location.purok_id !== undefined
      ) {
    ...
    if (
      ...
      Object.keys(updateData.demographics).length > 0
    ) {
      let demoFields = [];
      let demoValues = [];
    ...
    if (
      updateData.meetingSurvey &&
      Object.keys(updateData.meetingSurvey).length > 0
    ) {
      let meetingFields = [];
      let meetingValues = [];
...
export const destroy = async (req, res) => {
  ...
  try {
    ...
    await client.query("DELETE FROM sk_youth_attachments WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query("DELETE FROM sk_youth_household WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query(
      "DELETE FROM sk_youth_meeting_survey WHERE youth_id = $1",
      [youth_id],
    );
    await client.query("DELETE FROM sk_youth_survey WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query(
      "DELETE FROM sk_youth_demographics WHERE youth_id = $1",
      [youth_id],
    );
    await client.query("DELETE FROM sk_youth_info WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query("DELETE FROM sk_youth_gender WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query("DELETE FROM sk_youth_location WHERE youth_id = $1", [
      youth_id,
    ]);
    await client.query("DELETE FROM sk_youth_name WHERE youth_id = $1", [
      youth_id,
    ]);
